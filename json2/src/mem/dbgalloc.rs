...
unsafe impl Allocator for DefaultAllocator {
  fn allocate(&self, layout: Layout) -> Result<NonNull<[u8]>, AllocError> {
    ...
    {
      unsafe {
        let cstr: &CStr = CStr::from_bytes_with_nul(b"dbgAlloc: alloc %p %lu bytes\n\0").unwrap();
  ...
  fn allocate_zeroed(&self, layout: Layout) -> Result<NonNull<[u8]>, AllocError> {
    ...
    {
      unsafe {
        let cstr: &CStr = CStr::from_bytes_with_nul(b"dbgAlloc: allocZeroed %p %lu bytes\n\0").unwrap();
        printf(cstr.as_ptr(), ptr, layout.size());
      }
    }
  ...
  unsafe fn deallocate(&self, ptr: NonNull<u8>, layout: Layout) {
    ...
    {
      unsafe {
        let cstr: &CStr = CStr::from_bytes_with_nul(b"dbgAlloc: free %p %lu bytes\n\0").unwrap();
